...
import { useCreateCampusMutation } from "@/api/services/campus";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import Header from "@/components/header/header";
import PreviewLogo from "@/assets/root/preview";

const CreateCampusPage = () => {
  ...
  const [primaryColor, setPrimaryColor] = useState("#4b5563");
  ...
  const handleSubmit = () => {
    if (!name || !siteName || !provider || !logo) {
      setModalTitle("Error");
    ...
    if (documents.some((doc) => doc === undefined)) {
      setModalTitle("Error");
      setModalMessage("Please upload all documents or remove empty document fields.");
      setIsSuccessModal(false);
      setOpen(true);
      return;
    }
    createCampus.mutate({
      ...
      logo,
      document: documents.filter(Boolean) as File[],
    }, {
      onSuccess: (res) => {
        setModalTitle("Success");
        setModalMessage(res.message || "Campus created successfully");
        setIsSuccessModal(true);
        setOpen(true);
      },
      onError: (err) => {
        setModalTitle("Error");
        setModalMessage(err.message || "Failed to create campus");
        setIsSuccessModal(false);
        setOpen(true);
      }
    });
      ...
      <div className="space-y-4 px-4 sm:px-0">
          ...
          <CardContent className="p-4 text-[#5d5d5d]">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full">
              ...
              <Input
                ...
                disabled
                className="text-black focus-visible:ring-0 focus-visible:ring-offset-0 cursor-not-allowed w-auto sm:w-auto"
              />
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  />
                  {logo && (
                    <Button className='bg-neutral-50' type="button" variant="outline" onClick={() => setLogo(null)}>
                      Remove
                    </Button>
                  )}
                </div>
          ...
          <CardContent className="p-4 text-[#5d5d5d] flex flex-col gap-2">
            ...
            {documents.map((doc, idx) => (
              <div key={idx} className="flex gap-2 items-center">
                ...
                {doc && <span className="text-xs">{doc.name}</span>}
              </div>
            ))}
        ...
        <div className="flex justify-center sm:justify-end pb-4 px-4 sm:px-0">
          <Button
            ...
            disabled={createCampus.isLoading}
            className="w-full sm:w-auto min-w-[120px]"
          >
            {createCampus.isLoading ? "Submitting..." : "Submit"}
          </Button>
        </div>
      </div>
    </RootLayout>
