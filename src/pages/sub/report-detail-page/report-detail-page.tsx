...
import type { IPersonReport, IReport } from "@/types/model/report"
import { ChevronLeft, ChevronRight } from "lucide-react"
import type { IUpdateReportStatusRequest } from "@/types/request/report"
import CompleteReportModal from "@/components/complete-report-modal/complete-report-modal"
...
const ReportDetailPage = () => {
  ...
  const convertImageUrlToFile = async (imageUrl: string, fileName = "existing-image.jpg"): Promise<File> => {
    try {
  ...
  return (
    <SubLayout>
      ...
      <div className="space-y-4 px-4 sm:px-0">
        ...
        <Card>
          <CardContent className="p-4 text-[#5d5d5d]">
            ...
            <Input
              ...
              disabled={isViewMode}
              className={
                isViewMode ? "bg-gray-100" : "bg-neutral-50 focus-visible:ring-0 focus-visible:ring-offset-0 w-full"
              }
            />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4 text-[#5d5d5d]">
            ...
            <Select value={selectedAreaId} onValueChange={handleAreaSelect} disabled={isViewMode || isLoadingAreas}>
              <SelectTrigger
                className={`${isViewMode ? "bg-gray-100" : "bg-white focus:ring-0 focus:ring-offset-0"} border border-gray-200 w-full`}
              >
              ...
              <SelectContent className="bg-white border border-gray-200 shadow-lg w-full">
                {areasData?.data?.map((area) => (
                  <SelectItem key={area.id} value={area.id} className="hover:bg-gray-50 cursor-pointer">
                    {area.name}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            >
              ...
              <SelectContent className="bg-white border border-gray-200 shadow-lg w-full">
                {categoriesData?.data?.map((category) => (
                  <SelectItem key={category.id} value={category.id} className="hover:bg-gray-50 cursor-pointer">
                    {category.name}
                  </SelectItem>
                ))}
              </SelectContent>
              ...
              <div className="flex-1 order-1 sm:order-2">
                ...
                {!isViewMode ? (
                  <div className="flex flex-col sm:flex-row gap-2">
                    <Button className="bg-neutral-50" type="button" variant="outline" onClick={openCameraApp}>
                      Use Camera
                    </Button>
                ...
                <p className="mt-3 text-xs">File must be an image (.jpg / .png) or use camera to take photo</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {isEditMode && (
          <div className="flex justify-center sm:justify-end px-4 sm:px-0">
            ...
            >
              {isProcessingImage ? "Processing Image..." : updateReport.isLoading ? "Updating..." : "Update"}
            </Button>
          </div>
        )}
      ...
      {isEditMode && (
        <CameraModal isOpen={isCameraModalOpen} onClose={handleCameraClose} onCapture={handleCameraCapture} />
      )}
    </SubLayout>
  )
}
