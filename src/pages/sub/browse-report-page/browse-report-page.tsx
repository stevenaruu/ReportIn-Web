...
import DeleteReportModal from "@/components/delete-report-modal/delete-report-modal"
...
const BrowseReportPage = () => {
  ...
  const handleDeleteRemarkConfirm = (deletionRemark: string) => {
    deleteReport.mutate(
      { id: reportIdToDelete, data: { deletionRemark } },
      {
        onSuccess: (res) => {
  ...
  const handleDeleteWithRemark = (report: IReport) => {
  ...
  return (
    <SubLayout>
      <Modal
        ...
        onConfirm={modalType === "confirmDelete" ? () => {} : undefined}
      ...
      <DeleteReportModal
        ...
        onOpenChange={setDeleteReportModalOpen}
            ...
            {paginatedReports.length > 0 ? (
              paginatedReports.map((report) => (
                <ReportCard
                  ...
                  onDelete={handleDeleteWithRemark}
                />
              ))
            ) : (
